...
> reused.**
>
> **P13-S16's corrections against its own contract are amended in at
> `spec/CONTRACT_P13S16_PROJECTION.md` §7**, with `spec/EVIDENCE_P13S16_EXECUTION.md`
> holding the runs behind them. *(Execution reported six; §7's own review withdrew one as
> false — pin 8's line numbers were exact on the ratified tree and only stopped matching
> because this rung inserted lines above them.)* The two that bite hardest elsewhere:
> `cargo test --workspace` **truncates the failure set** at the first failing suite, so
> mutation work needs `--no-fail-fast`; and invariant 21 **abstains on dangling
> membership**, which is invariant 10's concern, so it is not the detector for the undo
> hole §0.6 attributed to it.
...
### 1.2 The live constraint — LIFTED 2026-08-09 by P13-S27 (`4df8e25`)

> **This section is a dated record, not a live rule.** The constraint it states held only
> until S27 landed, and S27 has landed. **The rest of §1.2 — down to §1.3 — describes the
> pre-S27 world and must not be followed.** What replaced it is in the **POST-S27 UPDATE
> at the top**, which is the only place that state is given; it is not restated here.
...
### 1.4 Chain state — SUPERSEDED 2026-08-09 by P13-S27 (`4df8e25`)

> **This whole section is a dated record.** The chain below, S27's status, and the three
> inherited items it lists all describe the pre-S27 tree; **the three items are
> discharged.** The current chain is in the **POST-S27 UPDATE at the top** and is not
> restated here.
...
Ledger: `spec/PASS13_CANDIDATES.md`. *(**Row states below are as of 2026-08-07 and are
superseded for S27, S16 and S26 — see the POST-S27 UPDATE at the top.** The ledger itself
is authoritative for every row, and the count of open rows is deliberately not restated
here.)* As of 2026-08-07 ten rows read **open**: S4, S8, S9,
...
> **SUPERSEDED 2026-08-09 — see the collision row of the POST-S27 UPDATE at the top.**
> §2.3's account of what blocks T1b is unaffected and still current.
...
```
cargo test --workspace                                   # count: see CLAUDE.md
cargo clippy --workspace --all-targets -- -D warnings     # clean
cargo fmt -p epiphany-bundle -p epiphany-textproj -p epiphany-testkit --check
```

**The expected count lives in `CLAUDE.md`'s *Green baseline* and is NOT repeated here —
changed 2026-08-09.** This document stated it in three places and S27 moved it, so all
three went stale together; a figure kept in four documents goes stale in three of them.
If the observed count differs from `CLAUDE.md`'s, reconcile that **before** starting new
work — this thread's whole method depends on a known-green baseline to mutate against.
...
6. **Reproduce the green baseline in §3.5 before starting new work.** The expected
   count is in `CLAUDE.md`'s *Green baseline* — **not restated here**, and it moved
...
`bundle.rs`.~~ **Superseded 2026-08-09 — see the collision row of the POST-S27 UPDATE.**

7. ~~**Spec thread — dispatch P13-S27.**~~ **DONE — see the POST-S27 UPDATE.**
...
9. Then **P13-S16** — ~~dispatchable the moment S27 lands~~ **superseded 2026-08-09; see
   the POST-S27 UPDATE** for its current state, and
   `spec/CONTRACT_P13S16_PROJECTION.md` for what it now requires.
