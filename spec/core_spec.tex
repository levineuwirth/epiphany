...
\begin{requirement}
  ...
  \&kind.canonical\_bytes())}. Because the identity is content-derived
...
\begin{requirement}
  ...
  underlying source is unchanged. It is derived by domain-separated
  BLAKE3 truncation with the reserved layout domain tag
  \texttt{"MUSCLOID"} over a key that depends on how the object is
  manifested:
  ...
  (Track~A), not the interchange track.
\end{requirement}
...
\begin{longtable}{p{2cm} p{2.5cm} p{9cm}}
  ...
  operation; and pinned the (non-canonical) \texttt{LayoutObjectId}
  derivation with a \texttt{MUSCLOID} tag. \emph{Fixes:} blob hashing
