...
\begin{requirement}
  ...
  \textbf{Minimal-tier slur rendering (ratified schema-major-2,
  Phase~F).} A \texttt{Slur} (\ref{sec:graph:slurs}) \MUST{} render
  ...
  non-\texttt{Solid} \texttt{SpanStyle} line (dashed, dotted) \MAY{}
  be deferred to a higher tier, but \MUST{} be surfaced (a layout
  diagnostic), never silently rendered solid. The curvature-computing
  algorithm and the dash rendering are engraving-specification
  concerns (\ref{sec:layoutir:forward}).
\end{requirement}
