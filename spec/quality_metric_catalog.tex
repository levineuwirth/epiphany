...
\emph{source} notates slurs, engraved by a solver that draws no slur
geometry, scores $\texttt{slur\_shape\_penalty} = 0.0$ under this rule ---
the axis sees no drawn slurs and finds nothing to penalize, even though
the output is arguably \emph{worse} than a badly-drawn slur. v0.1
deliberately pins vacuous-$0.0$: the metric axes evaluate the geometry the
solver produced, and \emph{rendering completeness} --- whether notated
...
The v0.1 reference pipeline draws no slur geometry (slurs exist logically,
not as curves), so this axis evaluates to $0.0$ today under the
vacuous-geometry rule --- the definition is pinned now so that the first
implementation to draw slurs is measured from its first release.
\end{rationale}
