...
\begin{requirement}
  ...
  version: \texttt{(0 11 0)}, the version of the companion it implements. It
...
\begin{requirement}
  ...
  version~0.11.0, neither a canonical operation nor canonical reduced state can
...
kind           ::= "(insert-event " bytes " " value ")"
               ...
               | "(set-canvas-layout-defaults " value ")"
               | "(set-spelling-precedence " value ")"
               | "(set-tuning-context " value ")"
...
\begin{longtable}{p{2cm} p{2.5cm} p{9cm}}
  ...
  migrate-on-read remain deferred, unchanged. \\
    \today & Chapter 5 & 0.9.0 --- The genesis settings setters reach the
  grammar. The \texttt{kind} production gains
  \texttt{"(set-canvas-layout-defaults " value ")"} and
  \texttt{"(set-spelling-precedence " value ")"}
  (\texttt{req:textproj:operation-vocabulary}), the second appended pair since
  the header was gated to a single version at 0.7.0.

  Same forcing reason as the 0.8.0 bump above: holding the version while
  ...
  migrate; a stale \texttt{TextProjection} chunk is regenerated, not
  converted. \\
    \today & Chapter 5 & 0.11.0 --- The genesis tuning-context setter reaches
  the grammar (genesis tranche G2b,
  \texttt{spec/CONTRACT\_GENESIS\_G2B\_TUNING.md}). The \texttt{kind}
  production gains \texttt{"(set-tuning-context " value ")"}
  (\texttt{req:textproj:operation-vocabulary}), the third appended kind since
  the header was gated to a single version at 0.7.0.
  ...
  grammars both claiming \texttt{(0 10 0)}. Cached projections at
  \texttt{(0 10 0)} do not migrate; a stale \texttt{TextProjection} chunk is
